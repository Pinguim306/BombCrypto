...
extends Node2D
## Pooled explosion (World layer, lit + shaken): the 6-frame cartoon boom
## sheet scaling up slightly, an additive ring, plus sparks and smoke from
## the shared particle presets.

@export var boom_scale_to := 1.15
@export var ring_from := 0.3
@export var ring_to := 1.9
@export var ring_seconds := 0.28
...
func _ready() -> void:
	visible = false
	boom.sprite_frames = Sheets.frames("chars/boom.png", 16.0)
	boom.sprite_frames.set_animation_loop(&"default", false)
...
func play(pos: Vector2, tier: int) -> void:
	...
	_tw.tween_property(boom, "scale", Vector2.ONE * lerpf(1.0, boom_scale_to, maxf(0.3, j)), 0.35)
